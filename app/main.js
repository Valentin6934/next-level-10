...
function renderAll(){renderSprintTwo($('sprintTwoRoot'),{openPage:showPage});renderSprintOne($('sprintOneRoot'));renderHome();renderSession();renderProgress();renderDay();renderCalendar();renderNutrition().catch(console.error);renderTracking();renderPain();renderNotifications();buildCoach();renderCoach();renderPersonalCockpit($('personalCockpitRoot'),{openPage:showPage,openConversation:()=>{showPage('nova-chat');loadNovaConversation()}});renderNovaCoach($('novaCoachRoot'),{openPage:showPage})}
